...
Según tu configuración actual, tienes:

- **Backend API**: `https://haidapi.stayarta.com`
- **Frontend**: `https://haida.stayarta.com`

**URLs necesarias para Microsoft OAuth**:
- **Redirect URI**: `https://haida.stayarta.com/auth/callback` (recomendado para flujo SPA)
- **Logout URI**: `https://haida.stayarta.com`
...
2. Si no existe, créala:
   ...
   - Redirect URI: **Web** → `https://haida.stayarta.com/auth/callback`
...
3. Agrega las siguientes URIs de redirección:
   ```
   https://haida.stayarta.com/auth/callback
...
2. Asegúrate de tener estos permisos de **Microsoft Graph**:
   ...
   - `Mail.Read` (Delegated) - Outlook
   - `Files.Read.All` (Delegated) - OneDrive
   - `Sites.Read.All` (Delegated) - SharePoint
   - `Team.ReadBasic.All` (Delegated) - Teams
...
# Valor: https://haida.stayarta.com/auth/callback

vercel env add ENTRA_AUTHORITY production
# Valor: https://login.microsoftonline.com/common
vercel env add ENTRA_TENANT_ID production
# Valor: <tu-tenant-id> (opcional si usas authority específico)
vercel env add ALLOWED_TENANT_IDS production
# Valor: <tenant-id-1,tenant-id-2> (opcional, restringe SSO por tenant)
vercel env add ENTRA_STATE_TTL_SEC production
# Valor: 600 (opcional, TTL del state)
vercel env add GRAPH_SCOPES production
# Valor: "openid profile email offline_access User.Read Mail.Read Files.Read.All Sites.Read.All Team.ReadBasic.All"
```
...
| `ENTRA_REDIRECT_URI` | `https://haida.stayarta.com/auth/callback` | Production |
...
export function AuthCallback() {
  ...
  useEffect(() => {
    ...
    if (code && state) {
      ...
      fetch('https://haidapi.stayarta.com/entra/callback', {
        method: 'POST',
      ...
      navigate('/login?error=no_code_or_state');
    }
...
// En tu página de Login
  const handleMicrosoftLogin = async () => {
    try {
      const response = await fetch('https://haidapi.stayarta.com/entra/login');
...
- [ ] Redirect URI configurado: `https://haida-frontend.vercel.app/auth/callback`
- [ ] Client ID copiado
