...
test.describe('API de Autenticación - HAIDA Backend', () => {
  ...
  test('Acceso a /auth/me con token válido', async ({ request }) => {
    ...
    expect(userData.email).toBe('test-haida@hiberus.com');
  ...
  test('Microsoft OAuth - Email caarevalo@hiberus.com debe ser válido para Entra', async ({ page }) => {
...
test.describe('Verificación en Base de Datos (Supabase)', () => {
  test('Consultar usuarios en Supabase después de registro', async ({ request }) => {
    ...
    const response = await request.get(`${SUPABASE_URL}/rest/v1/users?email=eq.test-haida@hiberus.com`, {
      headers: {
    ...
    if (response.ok()) {
      ...
      expect(users[0].email).toBe('test-haida@hiberus.com');
