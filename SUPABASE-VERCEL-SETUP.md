...
const client = new Client({
  connectionString: 'postgresql://postgres:Aupbag7.@db.wdebyxvtunromsnkqbrd.supabase.co:5432/postgres',
...
# Paste: postgresql://postgres:Aupbag7.@db.wdebyxvtunromsnkqbrd.supabase.co:5432/postgres
...
```powershell
# Quick test
node -e "const {Client}=require('pg'); const c=new Client({connectionString:'postgresql://postgres:Aupbag7.@db.wdebyxvtunromsnkqbrd.supabase.co:5432/postgres',ssl:{rejectUnauthorized:false}}); c.connect().then(()=>c.query('SELECT NOW()')).then(r=>console.log('✓',r.rows[0].now)).catch(console.error).finally(()=>c.end())"
```
...
# Backup database
pg_dump "postgresql://postgres:Aupbag7.@db.wdebyxvtunromsnkqbrd.supabase.co:5432/postgres" > backup.sql
