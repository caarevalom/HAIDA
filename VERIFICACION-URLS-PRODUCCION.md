...
|-----|------|----------|--------|
| https://haida.carlosarta.com | Frontend (custom) | Apunta al backend en lugar del frontend | ❌ MAL CONFIGURADO |
...
#### 1. https://back.carlosarta.com (Dominio personalizado)

**Endpoints verificados**:
```bash
✅ GET  /health       → 200 OK (0.29s)
   {"status":"healthy","timestamp":"2025-12-26T15:26:20.876004"}
...
✅ GET  /api/version  → 200 OK
   {"version":"2.0.0","environment":"production","build_date":"2025-12-16"}

✅ POST /api/auth/register → 200 OK (token generado)
   Request: {"email":"testprod@hiberus.com","password":"TestProd2025","full_name":"Test"}
   Response: {"access_token":"eyJhbGc...","token_type":"bearer","expires_in":86400,
              "user":{"id":"1ae6ccde...","email":"testprod@hiberus.com","role":"viewer"}}
...
**Estado**: ✅ Funcionando idénticamente a back.carlosarta.com
...
#### 1. https://haida.carlosarta.com (Dominio personalizado)
...
**Causa**: Dominio personalizado `haida.carlosarta.com` configurado en proyecto backend (haida-one) en lugar del frontend (haida-frontend)
...
### Problema 1: Dominio `haida.carlosarta.com` mal configurado ❌
...
**Impacto**: Alto - Usuarios que visiten haida.carlosarta.com verán JSON del backend en lugar de la UI

**Solución requerida**:
1. Ir a Vercel Dashboard → Proyecto `haida-one`
2. Settings → Domains
3. Remover dominio `haida.carlosarta.com`
4. Ir a proyecto `haida-frontend`
5. Settings → Domains
6. Agregar dominio `haida.carlosarta.com`
...
Dominio custom:   https://back.carlosarta.com
Estado:           ✅ FUNCIONANDO
...
Dominio custom:   (debería ser haida.carlosarta.com)
...
✅ POST /api/auth/register   → 200 OK (token emitido)
   - Usuario creado: testprod@hiberus.com
...
| Característica | back.carlosarta.com | haida.carlosarta.com | Esperado |
|----------------|---------------------|----------------------|----------|
...
2. Remover dominio: haida.carlosarta.com (si está asignado)
3. Confirmar que solo esté: back.carlosarta.com

4. Ir a https://vercel.com/carlos-arevalos-projects-cf7340ea/haida-frontend/settings/domains
5. Agregar dominio: haida.carlosarta.com
...
```bash
# Backend (no cambios)
curl -s https://back.carlosarta.com/api/health

# Frontend (debería devolver HTML)
curl -s https://haida.carlosarta.com/ | head -30
...
```bash
# Health check
curl -s https://back.carlosarta.com/health | python3 -m json.tool

# Status
curl -s https://back.carlosarta.com/api/status | python3 -m json.tool

# Create user
curl -s -X POST https://back.carlosarta.com/api/auth/register \
  -H "Content-Type: application/json" \
  -d '{"email":"test@example.com","password":"Pass1234","full_name":"Test User"}' \
  | python3 -m json.tool
```
...
curl -sI https://haida.carlosarta.com/ | grep -i content-type
# Esperado: content-type: text/html

# Obtener título de página
curl -s https://haida.carlosarta.com/ | grep -o '<title>[^<]*</title>'
...
### Acciones Requeridas
```
1. [ ] Remover haida.carlosarta.com de proyecto haida-one
2. [ ] Agregar haida.carlosarta.com a proyecto haida-frontend
...
6. [ ] Verificar que haida.carlosarta.com devuelva HTML
```

### URLs Correctas (Post-Fix)
```
Backend:   https://back.carlosarta.com
Frontend:  https://haida.carlosarta.com  (después de fix)
