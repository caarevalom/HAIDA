...
const calculateStats = (data: ReturnType<typeof useData>) => {
  ...
  const realTotalTestCases = suites.reduce(
    (acc, s) => acc + (typeof s.case_count === 'number' ? s.case_count : 0),
    0
  );
  ...
  const executionRate =
    realTotalTestCases > 0 ? Math.round((totalExecutions / realTotalTestCases) * 100) : 0;
  const criticalDefects = defects.filter((d) => d.severity === 'critical').length;

  return {
    activeProjects,
    totalTestCases: realTotalTestCases,
    executionRate,
    criticalDefects,
    passed,
    failed,
...
export function Dashboard() {
  ...
  const [cards, setCards] = useState([
    ...
    },
    {
      id: 'kpi2',
      type: 'kpi',
      title: t('kpi.coverage'),
      value: stats.totalTestCases,
      icon: 'layers',
      sub: 'Total Test Cases',
      trend: '+12% growth',
      trendUp: true,
    },
    {
    ...
    { id: 'chart1', type: 'chart-pie', title: 'Status Distribution', data: pieData },
    {
      id: 'chart2',
      type: 'chart-bar',
      title: 'Suites per Project',
      data: (data.projects || [])
        .slice(0, 4)
        .map((p) => ({
          name: p.key,
          value: (data.suites || []).filter((s) => s.project_id === p.id).length,
        })),
    },
    ...
    <div className="container mx-auto p-6 space-y-8 animate-in fade-in duration-500 pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 border-b border-border/40 pb-6">
        <div>
          ...
          <p className="text-muted-foreground">{dashboard.subtitleMessage}</p>
        </div>
          ...
          <Card
            ...
            className={cn(
              ...
              isEditMode
                ...
                : 'hover:shadow-lg hover:scale-[1.02] bg-card/80 backdrop-blur-sm'
            )}
              ...
              {card.type === 'kpi' && (
                <>
                  ...
                  <CardContent className="px-4 pb-4">
                    ...
                    <div className="font-semibold text-muted-foreground text-sm truncate">
                      {card.title}
                    </div>
      ...
      <Dialog open={!!expandedCard} onOpenChange={(open) => !open && setExpandedCard(null)}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            <DialogTitle>
              {expandedCard?.title || expandedCard?.project?.name || 'Widget Details'}
            </DialogTitle>
            <DialogDescription>Detailed view of the selected metric.</DialogDescription>
          </DialogHeader>
          <div className="h-[300px] w-full p-4 border rounded-md bg-muted/5 mt-2">
            ...
            {expandedCard?.type === 'kpi' && (
              <div className="flex flex-col items-center justify-center h-full gap-4">
                <div className="p-4 bg-primary/10 rounded-full">{getIcon(expandedCard.icon)}</div>
      ...
      <Dialog open={isAddWidgetOpen} onOpenChange={setIsAddWidgetOpen}>
        <DialogContent>
          ...
          <div className="grid grid-cols-2 gap-4 py-4">
            ...
            <Button
              variant="outline"
              className="h-24 flex flex-col gap-2 hover:bg-blue-50 hover:border-blue-500"
            ...
            <Button
              variant="outline"
              className="h-24 flex flex-col gap-2 hover:bg-green-50 hover:border-green-500"
            ...
            <Button
              variant="outline"
              className="h-24 flex flex-col gap-2 hover:bg-orange-50 hover:border-orange-500"
              onClick={() => addWidget('recent')}
            >
