...
export function Projects() {
  ...
  const projectWikiPages = wikiPages.filter(
    (p) => !selectedProjectId || p.projectId === selectedProjectId
  );
  ...
  const handleSaveWiki = () => {
    ...
    setWikiPages((prev) =>
      prev.map((p) => (p.id === selectedWikiId ? { ...p, content: wikiContent } : p))
    );
  ...
  const renderColumn = (status: TaskStatus, title: string, colorClass: string) => {
    ...
    return (
      <div className="flex-1 min-w-[300px] flex flex-col h-full bg-muted/20 rounded-xl border border-border/50 overflow-hidden">
        {/* Column Header */}
        <div
          className={cn('p-3 border-b flex items-center justify-between bg-muted/30', colorClass)}
        >
                ...
                <div className="flex justify-between items-start mb-2">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      <DropdownMenuItem onClick={() => moveTask(task.id, 'inprogress')}>
                        Move to In Progress
                      </DropdownMenuItem>
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-border/40 pb-4 shrink-0">
        <div>
          <div className="flex items-center gap-3 mb-1">
            ...
            {activeProject && (
              <Badge
                variant="outline"
                className="text-lg font-normal px-3 py-1 bg-background/50 backdrop-blur-sm"
              >
                {activeProject.key}
              </Badge>
            )}
          ...
          <p className="text-muted-foreground">
            Manage tasks, documentation, and team collaboration.
          </p>
        </div>
        ...
        <div className="flex items-center gap-3 overflow-x-auto max-w-full pb-2 md:pb-0 scrollbar-hide">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="rounded-full border border-dashed border-muted-foreground/30 text-muted-foreground"
        ...
        <div className="flex items-center justify-between shrink-0">
          ...
          <div className="flex items-center gap-3">
            ...
            <Button
              ...
              onClick={() => handleOpenTeam(selectedProjectId!)}
            >
