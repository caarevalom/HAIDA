...
3. **Completar formulario**:
   ```
   From Email: noreply@haida.carlosarta.com
...
Value: noreply@haida.carlosarta.com
Environments: ☑ Production ☑ Preview ☑ Development
...
# Variable 8
Name: EMAIL_BASE_URL
Value: https://haida.carlosarta.com
Environments: ☑ Production ☑ Preview ☑ Development
```
...
TOKEN=$(curl -s -X POST https://back.carlosarta.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"email":"carlosadmin@hiberus.com","password":"AdminCTB2025Pass"}' \
...
curl -X POST https://back.carlosarta.com/api/email/welcome \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d '{"email":"tu-email@example.com","name":"Test User"}'
```
...
SELECT * FROM users WHERE email = 'carlosadmin@hiberus.com';
