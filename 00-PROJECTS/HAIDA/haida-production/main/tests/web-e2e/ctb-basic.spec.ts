...
async function getInternalLinks(page, baseUrl, limit = 20) {
...
async function findPlpUrl(page, baseUrl) {
...
async function findPdpUrl(page, baseUrl) {
...
test.describe('CTB B2C - Home/Nav/Search', () => {
  ...
  test('TC_FOOTER_002 - Enlace condiciones de compra abre pagina', async ({ page, context }) => {
    await page.goto(`${BASE_URL}/es/`, { waitUntil: 'domcontentloaded' });
    await dismissCookieBanner(page);
    const footer = page.locator('footer');
    await expect(footer).toBeVisible();
