...
export function Executor() {
    ...
    if (nextStepIndex === stepStatuses.length - 1) {
      setTimeout(() => {
        if (currentCaseIndex < activeCases.length - 1) {
          ...
          toast.success('Case completed', { description: 'Advancing to next test case...' });
        } else {
          finishExecution();
        }
      }, 500);
  ...
  const finishExecution = () => {
    ...
    addExecution({
      ...
      status: 'passed', // Simplificado logicamente
      passed_count: activeCases.length,
      failed_count: 0,
        ...
        <Card className="border-2 border-dashed border-primary/20 shadow-xl bg-card/50 backdrop-blur-sm">
          <CardHeader className="text-center pb-8 pt-10">
            ...
            <CardDescription className="text-lg mt-2">
              Execute your test plans and log results in real-time
            </CardDescription>
          </CardHeader>
                  ...
                  <SelectContent>
                    {projectSuites.map((s) => (
                      <SelectItem key={s.id} value={s.id}>
                        {s.name} ({cases.filter((c) => c.suite_id === s.id).length} cases)
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
    ...
    return (
      <div className="container mx-auto p-6 flex flex-col items-center justify-center h-[80vh] animate-in fade-in">
        ...
        <p className="text-xl text-muted-foreground mb-8 text-center max-w-md">
          The suite <strong>{activeSuite?.name}</strong> has been executed. Results have been saved
          to the history.
        </p>
        <div className="flex gap-4">
          <Button variant="outline" onClick={() => setExecStatus('idle')}>
            Back to Runner
          </Button>
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-xs text-muted-foreground">
              Running {currentCase?.title} ({currentCaseIndex + 1}/{activeCases.length})
            </span>
          </div>
        </div>
                    ...
                    <CardContent className="p-4 flex gap-4">
                      ...
                      {isActive && (
                        <div className="flex flex-col gap-2 justify-center ml-2">
                          <Button
                            size="sm"
                            className="bg-green-600 hover:bg-green-700 w-24 shadow-sm"
                          ...
                          <Button
                            ...
                            variant="secondary"
                            className="w-24"
                            onClick={() => handleStepAction('blocked')}
                          >
        ...
        <div className="absolute inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <Card className="w-full max-w-lg shadow-2xl border-red-200 dark:border-red-900 animate-in fade-in zoom-in-95">
            <CardHeader className="bg-red-50 dark:bg-red-900/20 pb-4">
              ...
              <CardDescription>Describe actual result vs expected result.</CardDescription>
            </CardHeader>
