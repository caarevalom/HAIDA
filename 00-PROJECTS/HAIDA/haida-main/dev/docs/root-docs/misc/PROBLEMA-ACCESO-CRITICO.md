...
3. Ve a: **https://haida.carlosarta.com**
4. Introduce:
   ```
   Email: hola@stayarta.com
...
4. Click derecho en `https://haida.carlosarta.com`
5. Selecciona **"Clear site data"**
6. **Cierra TODAS las ventanas del navegador**
7. Abre de nuevo y accede a https://haida.carlosarta.com
...
**Síntoma**:
```bash
curl https://haida.carlosarta.com
# Retorna:
{
  "status": "healthy",
  "service": "HAIDA API",
...
**Causa Raíz**:
- El dominio `haida.carlosarta.com` está apuntando al proyecto incorrecto en Vercel
...
# Remover dominio
vercel domains rm haida.carlosarta.com --project haida-one

# Agregar a proyecto correcto
vercel domains add haida.carlosarta.com --project haida-frontend
```
...
```
Frontend (haida.carlosarta.com):
  ❌ Sirve backend API (JSON)
  ❌ No carga React app

Backend (back.carlosarta.com):
  ✅ Operativo
  ✅ APIs funcionan
...
- [ ] Remover `haida.carlosarta.com` de `haida-one`
- [ ] Asignar `haida.carlosarta.com` a proyecto frontend correcto
- [ ] Esperar propagación DNS (1-2 min)
- [ ] Verificar: `curl https://haida.carlosarta.com | grep root`
...
- [ ] Abrir en navegador: https://haida.carlosarta.com
...
**Dominios esperados**:
- Frontend: `haida.carlosarta.com` ← **Corregir esto**
- Backend: `back.carlosarta.com` ← **Ya correcto**
...
### Solución
1. Reasignar dominio `haida.carlosarta.com` al proyecto frontend correcto en Vercel
