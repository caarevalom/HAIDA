...
const isValidClaudeArgs = (args: any): args is ClaudeRequest =>
  typeof args === 'object' &&
  args !== null &&
...
const isValidSubagentArgs = (args: any): args is SubagentRequest =>
  typeof args === 'object' &&
  args !== null &&
...
class ClaudeServer {
  ...
  private async handleClaudeOpus(args: any) {
    if (!isValidClaudeArgs(args)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Invalid Claude Opus arguments'
      );
    }
  ...
  private async handleSubagent(agentType: string, args: any) {
    if (!isValidSubagentArgs(args)) {
      throw new McpError(
        ErrorCode.InvalidParams,
        'Invalid subagent arguments'
      );
    }
