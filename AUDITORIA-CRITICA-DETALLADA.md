...
│    Solución:
│       if (-not $nodeVer) { Write-Error_ "node -v falló"; exit 1 }
│       if (-not $npmVer) { Write-Error_ "npm -v falló"; exit 1 }
│    Esfuerzo: 10 min
...
│       ├─ Valida headers presentes (TEST_ID, TIPO_PRUEBA, etc)
│       ├─ Valida tipos de dato (PRIORIDAD in P0-P3)
│       ├─ Valida formatos (TEST_ID = TC_MODULO_###)
